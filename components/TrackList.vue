...
<template>
    ...
    <div class="items__containner" v-for="(item,index) in list">
        <div class="song__item" @click="playMusic($event,item)">
            ...
            <div class="playlist__name">{{ item.name }}</div>
            <div class="playlist__name" style="width: 80px;">{{ formatSingers(item.singer) }}</div>
            <div style="margin:0 50px;">{{ formatTime(item.duration) }}</div>
            <svg
                class="mini-icon"
...
.containner {
    ...
    .tracklist__containner {
        ...
        img {
            ...
            border-radius: 10px;
            @include useTheme {
                box-shadow: 5px 5px 5px getVar("selectedColor");
            }
        }
        .info__containner {
            font-size:25px;
