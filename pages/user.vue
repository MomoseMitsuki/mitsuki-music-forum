...
const ProhibitStrings = ["", "$default", "$love"]
const showLists = () => {
    // todo: show the user's playlists
    isShowLists.value = !isShowLists.value
}
const addList = () => (isInputListName.value = true)
const submitUserList = async ($event: Event) => {
    $event.preventDefault()
    if (ProhibitStrings.includes(inputValue.value)) {
        inputValue.value = ""
        isInputListName.value = false
        return
    }
    const data = {
...
const list = computed<MusicList>(() => {
    if (cur_item.value === "收藏歌单") {
        return $love.value!
    } else {
        for (const list of userStore.$lists) {
            if (cur_item.value === list.name) {
                return list
            }
        }
    }
    ...
    <div class="usercard__containner">
        <div class="containner__Info">
            ...
            <div class="containner__list">
                ...
                <div v-if="isShowLists">
                    <div
                        class="list__item"
                        v-for="item in userStore.$lists"
                        @click="cur_item = item.name"
                        :class="{ list__item_selected: cur_item === item.name }"
                    >
                        <svg
                            viewBox="0 0 1024 1024"
                            xmlns="http://www.w3.org/2000/svg"
