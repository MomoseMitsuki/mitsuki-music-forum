...
export const useUiStatusStore = defineStore("uiStatusStore", () => {
    ...
    const jump = (e:MouseEvent) => {
        if (listEle.value){
            ...
            const endRect = listEle.value.getBoundingClientRect()
            const startX = Math.round(startRect.left + startRect.width / 4 * 3)
            ...
            (div.children[0] as HTMLElement).style.transform = `translateY(${startY}px)`
            document.body.appendChild(div)
            requestAnimationFrame(() => {
                // div.style.transform = `translate(${endX}px,${endY}px)`;
                div.style.transform = `translateX(${endX}px)`;
                (div.children[0] as HTMLElement).style.transform = `translateY(${endY}px)`
            })
